...
export default function MT4AccountsTab({ onUpdate }) {
  ...
  const loadAccounts = async () => {
    setIsLoading(true);
    try {
      const accountsList = await base44.entities.MT4Account.list();
      setAccounts(accountsList);
    } catch (error) {
      console.error("Failed to load MT4 accounts:", error);
      toast.error("Failed to load MT4 accounts");
    }
    setIsLoading(false);
  };
  ...
  const handleConnect = async () => {
    ...
    try {
      const response = await base44.functions.invoke('mt4Connect', newAccount);
      
      if (response.data.success) {
        toast.success("Successfully connected to MT4 account");
        setNewAccount({
          login_id: "",
          password: "",
          server_name: "",
        ...
        toast.error(response.data.error || "Failed to connect to MT4");
      }
    } catch (error) {
      console.error("Connection error:", error);
      toast.error("Failed to connect to MT4");
    }
  ...
  const handleDelete = async (accountId) => {
    try {
      await base44.entities.MT4Account.delete(accountId);
    ...
    <div className="space-y-6">
      ...
      {isAdding && (
        ...
        >
          <Card className="bg-slate-800/50 border-slate-700 backdrop-blur">
            <CardHeader>
              <CardTitle className="text-white">Connect MT4 Account</CardTitle>
            </CardHeader>
            <CardContent>
              <div className="space-y-4">
                <div className="grid md:grid-cols-2 gap-4">
                  <div>
                    ...
                    <Input
                      ...
                      placeholder="e.g., My MT4 Account"
                ...
                <div className="bg-yellow-900/20 border border-yellow-500/30 rounded-lg p-4">
                  <p className="text-yellow-200 text-sm">
                    ⚠️ <strong>Note:</strong> This is a simulated MT4 connection. In production, actual MT4 integration would be implemented.
                  </p>
                </div>
      ...
      {accounts.length > 0 ? (
        ...
        <Card className="bg-slate-800/50 border-slate-700 backdrop-blur">
          <CardContent className="p-12 text-center">
            ...
            <h3 className="text-xl font-semibold text-white mb-2">No MT4 Accounts Connected</h3>
            <p className="text-slate-400 mb-4">
              Connect your MT4 account to start trading and monitoring positions
            </p>
            <Button
              onClick={() => setIsAdding(true)}
              className="bg-blue-600 hover:bg-blue-700"
